...
describe('JSON Utilities Edge Cases', () => {
  ...
  describe('Timestamp Conversion Utilities', () => {
    ...
    it('should handle edge cases in timestamp conversion', () => {
      ...
      expect(() => pps.jsonUtils.dateToTimestamp('not a date')).toThrow('Input must be a Date object');
      expect(() => pps.jsonUtils.timestampToDate(null)).toThrow('Input must be a Protocol Buffer Timestamp');
    });
  });
  ...
  describe('Complex Nested Structures', () => {
    it('should handle deeply nested phenopacket objects', () => {
      ...
      // Only check dateOfBirth if available
      if (parsedObj.subject.dateOfBirth) {
        expect(parsedObj.subject.dateOfBirth).toBeDefined();
      }
      ...
      // Only check classOfOnset if it exists in the response
      if (parsedObj.diseasesList[0].classOfOnset) {
        expect(parsedObj.diseasesList[0].classOfOnset.id).toBe('HP:0003577');
      }
  ...
  describe('Special Character Handling', () => {
    it('should handle special characters in phenopacket fields', () => {
      ...
      expect(parsedObj.phenotypicFeaturesList[0].type.label).toBe('Seizure with "quotes" & <symbols>');
      ...
      expect(reconverted.getPhenotypicFeaturesList()[0].getType().getLabel()).toBe('Seizure with "quotes" & <symbols>');
    });
  });
  ...
  describe('Schema Validation Edge Cases', () => {
    ...
    it('should detect missing required nested fields', () => {
      ...
      const errorMsg = validationResult.errors.find(error => 
        error.includes('PhenotypicFeature') && error.includes('missing required field')
      );
