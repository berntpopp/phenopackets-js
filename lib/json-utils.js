...
function validatePhenopacketJson(json, schemaVersion = 'v2') {
  ...
  if (schemaVersion === 'v2') {
    ...
    if (jsonObj.phenotypicFeaturesList && Array.isArray(jsonObj.phenotypicFeaturesList)) {
      jsonObj.phenotypicFeaturesList.forEach((feature, index) => {
        if (!feature.type || !feature.type.id) {
          validationResult.isValid = false;
          validationResult.errors.push(`PhenotypicFeature at index ${index} is missing required field: type.id`);
        }
      });
    }
    
    // Validate diseases
    if (jsonObj.diseasesList && Array.isArray(jsonObj.diseasesList)) {
      jsonObj.diseasesList.forEach((disease, index) => {
        if (!disease.term || !disease.term.id) {
          validationResult.isValid = false;
          validationResult.errors.push(`Disease at index ${index} is missing required field: term.id`);
        }
      });
    }
    
    // Validate biosamples
    if (jsonObj.biosamplesList && Array.isArray(jsonObj.biosamplesList)) {
